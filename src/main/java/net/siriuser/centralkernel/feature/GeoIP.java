package net.siriuser.centralkernel.feature;
...
public class GeoIP {
     ...
     * @param plugin
     */
    public GeoIP(final CentralKernelCore plugin){
        instance = this;
        this.plugin = plugin;
    }
    ...
    public void init(){
        ...
        final ConfigurationManager conf = PluginHelper.getInstance().getConfig();
    ...
    public String getGeoIpString(final InetAddress addr, final boolean simple){
        ...
        if (simple){
            if (PluginHelper.getInstance().getConfig().getUseCityDB()){
        ...
        else if (PluginHelper.getInstance().getConfig().getUseCityDB()){
